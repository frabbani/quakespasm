...
cvar_t external_ents =
  { "external_ents", "1", CVAR_ARCHIVE };
cvar_t external_vis =
  { "external_vis", "1", CVAR_ARCHIVE };
...
void Mod_Init( void ){
  ...
  r_notexture_mip = (texture_t*) Hunk_AllocName( sizeof(texture_t),
                                                 "r_notexture_mip" );
  strcpy( r_notexture_mip->name, "notexture" );
  r_notexture_mip->height = r_notexture_mip->width = 32;

  r_notexture_mip2 = (texture_t*) Hunk_AllocName( sizeof(texture_t),
                                                  "r_notexture_mip2" );
...
byte* Mod_DecompressVis( byte *in, qmodel_t *model ){
  ...
  if( mod_decompressed == NULL || row > mod_decompressed_capacity ){
    ...
    mod_decompressed = (byte*) realloc( mod_decompressed,
    ...
    if( !isDedicated ) //no texture uploading for dedicated server
    {
      if( !q_strncasecmp( tx->name, "sky", 3 ) ) //sky texture //also note -- was Q_strncmp, changed to match qbsp
        ...
        else //use the texture from the bsp file
        {
          ...
          tx->gltexture = TexMgr_LoadImage( loadmodel, texturename, tx->width,
                                            tx->height, SRC_INDEXED,
        ...
        q_snprintf( texturename, sizeof( texturename ), "%s_warp",
                    texturename );
      ...
      else //regular texture
      {
        ...
        if( data ) //load external image
        {
          tx->gltexture = TexMgr_LoadImage( loadmodel, filename, fwidth,
                                            fheight, SRC_RGBA, data, filename,
...
void Mod_LoadLighting( lump_t *l ){
  ...
  if( loadmodel->bspversion == BSPVERSION_QUAKE64 ){
    ...
    loadmodel->lightdata = (byte*) Hunk_AllocName( ( l->filelen / 2 ) * 3,
                                                   litfilename );
...
void Mod_LoadEntities( lump_t *l ){
  ...
  q_snprintf( entfilename, sizeof( entfilename ), "%s@%04x.ent", basemapname,
              crc );
...
void Mod_ProcessLeafs_S( dsleaf_t *in, int filelen ){
  ...
  for( i = 0; i < count; i++, in++, out++ ){
    ...
    out->firstmarksurface = loadmodel->marksurfaces
        + (unsigned short) LittleShort( in->firstmarksurface ); //johnfitz -- unsigned short
...
void Mod_BoundsFromClipNode( qmodel_t *mod, int hull, int nodenum ){
  ...
  switch( plane->type ){
    ...
    case PLANE_X:
      if( plane->signbits == 1 )
        mod->clipmins[0] = q_min(
            mod->clipmins[0], -plane->dist - mod->hulls[hull].clip_mins[0] );
      else
    ...
    case PLANE_Y:
      if( plane->signbits == 2 )
        mod->clipmins[1] = q_min(
            mod->clipmins[1], -plane->dist - mod->hulls[hull].clip_mins[1] );
      else
    ...
    case PLANE_Z:
      if( plane->signbits == 4 )
        mod->clipmins[2] = q_min(
            mod->clipmins[2], -plane->dist - mod->hulls[hull].clip_mins[2] );
      else
  ...
  for( i = 0; i < numskins; i++ ){
    if( pskintype->type == ALIAS_SKIN_SINGLE ){
      ...
      for( j = 0; j < groupskins; j++ ){
        ...
        //johnfitz -- rewritten
        q_snprintf( name, sizeof( name ), "%s:frame%i_%i", loadmodel->name, i,
                    j );
...
//
  ...
  pskintype = (daliasskintype_t*) Mod_LoadAllSkins( pheader->numskins,
                                                    pskintype );
...
void* Mod_LoadSpriteGroup( void *pin, mspriteframe_t **ppframe, int framenum ){
  ...
  *ppframe = (mspriteframe_t*) pspritegroup;
  ...
  poutintervals = (float*) Hunk_AllocName( numframes * sizeof(float),
                                           loadname );
