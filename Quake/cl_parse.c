...
void CL_ParseLocalSound( void ){
  ...
  field_mask = MSG_ReadByte();
  sound_num =
      ( field_mask & SND_LARGESOUND ) ? MSG_ReadShort() : MSG_ReadByte();
  if( sound_num >= MAX_SOUNDS )
...
//
  ...
  while( 1 ){
    ...
    switch( cmd ){
      default:
        //	CL_DumpPacket ();
        Host_Error( "Illegible server message %d (previous was %s)", cmd,
      ...
      case svc_lightstyle:
        ...
        if( cl_lightstyle[i].length ){
          ...
          for( j = 0; j < cl_lightstyle[i].length; j++ ){
            ...
            cl_lightstyle[i].peak = q_max( cl_lightstyle[i].peak,
