...
cvar_t r_nolerp_list = { "r_nolerp_list",
    ...
    CVAR_NONE };
cvar_t r_noshadow_list = { "r_noshadow_list", "progs/flame2.mdl,progs/flame.mdl,progs/bolt1.mdl,progs/bolt2.mdl,progs/bolt3.mdl,progs/laser.mdl",
CVAR_NONE };
...
// create render-to-texture texture if needed
  if (!r_gamma_texture) {
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA8, r_gamma_texture_width, r_gamma_texture_height, 0, GL_BGRA,
...
void R_SetupGL(void) {
  ...
  Mat4 P = mat4Persp(aspect, r_fovx * M_PI_DIV_180, 0.1f, 3000.0f);
  ...
  Vec3 p, r, l, u;
  p = vec3_(r_refdef.vieworg[0], r_refdef.vieworg[1], r_refdef.vieworg[2]);
  l = vec3L();
  u = vec3U();
  r = vec3R();

  l = vec3Rot(l, u, r_refdef.viewangles[YAW] * M_PI_DIV_180);
  r = vec3Rot(r, u, r_refdef.viewangles[YAW] * M_PI_DIV_180);

  l = vec3Rot(l, r, -(r_refdef.viewangles[PITCH] + 0.0f) * M_PI_DIV_180);
  u = vec3Rot(u, r, -(r_refdef.viewangles[PITCH] + 0.0f) * M_PI_DIV_180);

  Mat4 V = mat4View(p, r, l, u);
  // glLoadIdentity();
...
//	glBlendFunc (GL_ONE, GL_ONE);
  ...
  if (r_drawentities.value) {
    ...
    // viewmodel
    currententity = &cl.viewent;
    if (r_drawviewmodel.value && !chase_active.value && cl.stats[STAT_HEALTH] > 0 && !(cl.items & IT_INVISIBILITY) && currententity->model && currententity->model->type == mod_alias) {
      glDepthRange(0, 0.3);
      R_DrawAliasModel_ShowTris(currententity);
      glDepthRange(0, 1);
    }
...
void R_ScaleView(void) {
  ...
  if (r_scaleview_texture_width < srcw || r_scaleview_texture_height < srch) {
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, r_scaleview_texture_width, r_scaleview_texture_height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
    NULL);
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  }
...
void R_RenderView(void) {
  ...
  if (r_pos.value)
    Con_Printf("x %i y %i z %i (pitch %i yaw %i roll %i)\n", (int) cl_entities[cl.viewentity].origin[0], (int) cl_entities[cl.viewentity].origin[1], (int) cl_entities[cl.viewentity].origin[2],
               (int) cl.viewangles[PITCH], (int) cl.viewangles[YAW], (int) cl.viewangles[ROLL]);
  else if (r_speeds.value == 2)
    Con_Printf("%3i ms  %4i/%4i wpoly %4i/%4i epoly %3i lmap %4i/%4i sky %1.1f mtex\n", (int) ((time2 - time1) * 1000), rs_brushpolys, rs_brushpasses, rs_aliaspolys, rs_aliaspasses,
               rs_dynamiclightmaps, rs_skypolys, rs_skypasses, TexMgr_FrameUsage());
