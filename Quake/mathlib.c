...
Transform make_transform(vec3 p, vec3 angles) {
  ...
  AngleVectors(t.angles.xyz, t.basis.l.xyz, t.basis.r.xyz, t.basis.u.xyz);
...
vec3 transform_vec3(const Transform *transform, vec3 p, TransformSpace space, qboolean direction_only) {
  ...
  if (space == LocalSpace) {
    ...
    vec3 lru[3];
    for (int i = 0; i < 3; i++) {
      lru[i] = v3scale(transform->basis.lru[i], p.xyz[i]);
    }
