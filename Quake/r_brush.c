...
int AllocBlock( int w, int h, int *x, int *y ){
  ...
  for( texnum = last_lightmap_allocated; texnum < MAX_SANITY_LIGHTMAPS;
    ...
    if( texnum == lightmap_count ){
      ...
      lightmaps[texnum].data = (byte*) calloc(
          1, 4 * LMBLOCK_WIDTH * LMBLOCK_HEIGHT );
...
void GL_CreateSurfaceLightmap( msurface_t *surf ){
  ...
  surf->lightmaptexturenum = AllocBlock( smax, tmax, &surf->light_s,
                                         &surf->light_t );
...
void GL_BuildLightmaps( void ){
  ...
  for( i = 0; i < lightmap_count; i++ ){
    ...
    LMBLOCK_HEIGHT,
                                    SRC_LIGHTMAP, lm->data, "",
                                    (src_offset_t) lm->data,
...
static void R_UploadLightmap( int lmap ){
  ...
  glTexSubImage2D(
      GL_TEXTURE_2D, 0, 0, lm->rectchange.t, LMBLOCK_WIDTH, lm->rectchange.h,
      gl_lightmap_format,
      GL_UNSIGNED_BYTE,
