...
int StripLength(int starttri, int startv) {
  ...
  nexttri: for (j = starttri + 1, check = &triangles[starttri + 1]; j < pheader->numtris; j++, check++) {
...
// upload indices buffer
  ...
  glBufferData( GL_ELEMENT_ARRAY_BUFFER, hdr->numindexes * sizeof(unsigned short), indexes,
  GL_STATIC_DRAW);
