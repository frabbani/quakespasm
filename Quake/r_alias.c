...
void GLAlias_CreateShaders(void) {
  const glsl_attrib_binding_t bindings[] = { { "TexCoords", texCoordsAttrIndex }, { "Pose1Vert", pose1VertexAttrIndex }, { "Pose1Normal", pose1NormalAttrIndex }, { "Pose2Vert", pose2VertexAttrIndex },
...
void GL_DrawAliasFrame(aliashdr_t *paliashdr, lerpdata_t lerpdata) {
  ...
  while (1) {
    ...
    do {
      ...
      if (lerping) {
        glVertex3f(verts1->v[0] * iblend + verts2->v[0] * blend, verts1->v[1] * iblend + verts2->v[1] * blend, verts1->v[2] * iblend + verts2->v[2] * blend);
        verts1++;
        verts2++;
      } else {
...
void R_DrawAliasModel(entity_t *e) {
  ...
  T.e11 = paliashdr->scale[1] * fovscale;
  T.e22 = paliashdr->scale[2] * fovscale;
  T.e03 = paliashdr->scale_origin[0];
...
void GL_DrawAliasShadow(entity_t *e) {
  float shadowmatrix[16] = { 1, 0, 0, 0, 0, 1, 0, 0, SHADOW_SKEW_X, SHADOW_SKEW_Y, SHADOW_VSCALE, 0, 0, 0, SHADOW_HEIGHT, 1 };
  float lheight;
  aliashdr_t *paliashdr;
  lerpdata_t lerpdata;
