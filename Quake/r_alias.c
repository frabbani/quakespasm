...
void GLAlias_CreateShaders( void ){
  ...
  r_alias_program = GL_CreateProgram(
      vertSource, fragSource, sizeof( bindings ) / sizeof( bindings[0] ),
      bindings );
  ...
  if( r_alias_program != 0 ){
    ...
    fullbrightTexLoc = GL_GetUniformLocation( &r_alias_program,
                                              "FullbrightTex" );
    useFullbrightTexLoc = GL_GetUniformLocation( &r_alias_program,
                                                 "UseFullbrightTex" );
    useOverbrightLoc = GL_GetUniformLocation( &r_alias_program,
                                              "UseOverbright" );
...
void GL_DrawAliasFrame_GLSL( aliashdr_t *paliashdr, lerpdata_t lerpdata,
  ...
  GL_BindBuffer( GL_ELEMENT_ARRAY_BUFFER,
                 currententity->model->meshindexesvbo );
      ...
      if( shading ){
        if( r_drawflat_cheatsafe ){
          ...
          vertcolor[0] = ( shadedots[verts1->lightnormalindex] * iblend
              + shadedots[verts2->lightnormalindex] * blend ) * lightcolor[0];
          vertcolor[1] = ( shadedots[verts1->lightnormalindex] * iblend
              + shadedots[verts2->lightnormalindex] * blend ) * lightcolor[1];
          vertcolor[2] = ( shadedots[verts1->lightnormalindex] * iblend
              + shadedots[verts2->lightnormalindex] * blend ) * lightcolor[2];
...
void R_SetupEntityTransform( entity_t *e, lerpdata_t *lerpdata ){
  ...
  if( r_lerpmove.value && e != &cl.viewent && e->lerpflags & LERP_MOVESTEP ){
    if( e->lerpflags & LERP_FINISH )
      blend = CLAMP(
          0,
          ( cl.time - e->movelerpstart ) / ( e->lerpfinish - e->movelerpstart ),
          1 );
    else
...
void R_SetupAliasLighting( entity_t *e ){
  ...
  quantizedangle = ( (int) ( e->angles[1] * ( SHADEDOT_QUANT / 360.0 ) ) )
      & ( SHADEDOT_QUANT - 1 );
...
void GL_DrawAliasShadow( entity_t *e ){
  float shadowmatrix[16] =
    { 1, 0, 0, 0, 0, 1, 0, 0,
    SHADOW_SKEW_X, SHADOW_SKEW_Y, SHADOW_VSCALE, 0, 0, 0, SHADOW_HEIGHT, 1 };
  float lheight;
  aliashdr_t *paliashdr;
  lerpdata_t lerpdata;
