...
RL_agent_t RL_init(RL_type_t type /* RL type SARSA or Q-LEARN*/, double alpha /*Bellman learning rate (0 to 1)*/, double epsilon /*greedy exploration rate*/, double gamma /*discount factor (0 to 1)*/,
                   const NN_info_t *nn_info, RL_set_input_cb set, RL_reward_cb reward, RL_act_cb act, RL_agent_state_t state);

void RL_term(RL_agent_t *agent_ptr);
double RL_step(RL_agent_t agent);  // executes one RL update step using the specified algorithm (SARSA or Q-Learning)
