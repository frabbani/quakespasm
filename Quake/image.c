...
qboolean Image_WriteBMP(const char *name, byte *data, int width, int height, int bpp, qboolean upsidedown) {
  if (bpp != 24 && bpp != 32) {
    Con_Printf("Image_WriteBMP: unsupported bpp %d\n", bpp);
    return false;
  }
  bpp = 24 == bpp ? 3 : 4;  //bits-per-pixel -> bytes-per-pixel
  ...
  // Allocate and build the BMP data
  byte *bmp_data = (byte*) Z_Malloc(file_header.bfSize);
  if (!bmp_data) {
    Con_Printf("Image_WriteBMP: failed to allocate %d bytes\n", file_header.bfSize);
    return false;
  }

  // Write headers
  memcpy(bmp_data, &file_header, sizeof(file_header));
  memcpy(bmp_data + sizeof(file_header), &info_header, sizeof(info_header));

  // Write pixel data
  byte *dst = bmp_data + file_header.bfOffBits;
  for (int y = 0; y < height; y++) {
    int src_y = upsidedown ? (height - 1 - y) : y;
    byte *src_row = data + src_y * width * bpp;

    for (int x = 0; x < width; x++) {
      byte *src_pixel = src_row + x * bpp;
      dst[x * 3 + 0] = src_pixel[2];  // B
      dst[x * 3 + 1] = src_pixel[1];  // G
      dst[x * 3 + 2] = src_pixel[0];  // R
    }
  ...
  // Save file
  COM_WriteFile(name, bmp_data, file_header.bfSize);
  Z_Free(bmp_data);
